...
{
    // 사용자 회원가입, 로그인, 로그아웃
    //[Authorize] // 권한이 있어야만 할 수 있다. // 레지스터, 로그인은 권한 없이 가능
    public class AccountController : Controller
    {
        ...
        //[AllowAnonymous] // 아무나 들어와서 회원가입 가능
        [HttpGet]
        ...
        [HttpPost]
        public async Task<IActionResult> Profile(RegisterModel model) // 프로필 변경
        {
            if (ModelState.IsValid) 
            {
                var user = await _userManager.FindByNameAsync(model.Email); // 값을 찾은 다음 업데이트 해야됨!
            ...
            return View(model); // 프로필변경 실패하면 화면 그대로 유지
        }
    }
}
